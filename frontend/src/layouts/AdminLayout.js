...
const AdminLayout = ({ children }) => {
  ...
  const navigation = [
    ...
    { name: 'Manager Chat', href: '/admin/chat', icon: MessageSquare },
  ];
          ...
          <div className="fixed inset-y-0 left-0 w-72 bg-white dark:bg-slate-950 shadow-2xl flex flex-col">
            <div className="flex items-center justify-between h-16 px-4 border-b border-gray-200 dark:border-slate-800">
              <div className="flex items-center space-x-3">
                <div className="h-10 w-10 rounded-2xl bg-gradient-to-br from-purple-600 to-indigo-600 text-white flex items-center justify-center font-semibold text-sm">
                  <ShieldCheck className="w-5 h-5" />
                </div>
                <div className="leading-snug">
                  <p className="text-base font-semibold text-gray-900 dark:text-slate-100">Admin Portal</p>
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center px-3 py-3 text-[15px] font-medium rounded-xl transition-colors ${
                      isActive
                        ? 'bg-purple-100 text-purple-700 border-r-2 border-purple-500 dark:bg-slate-800 dark:text-purple-300'
            ...
            <div className="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
              <div className="flex items-center">
                ...
                <div className="ml-2">
                  <p className="text-sm font-semibold text-gray-900 dark:text-slate-100">
                    {navigation.find(item => item.href === location.pathname || (item.href !== '/admin' && location.pathname.startsWith(item.href)))?.name || 'Admin Dashboard'}
                  </p>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="hidden sm:flex items-center rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2.5 py-1.5 shadow-sm">
                  <span className="h-7 w-7 rounded-full bg-gradient-to-br from-purple-600 to-indigo-600 text-white text-sm font-semibold flex items-center justify-center mr-2">
                    {userInitials}
                  </span>
                  <span className="text-sm font-medium text-gray-900 dark:text-white max-w-[10rem] truncate">{user?.name || user?.full_name || 'Admin'}</span>
                </div>
              </div>
            </div>
          </div>
